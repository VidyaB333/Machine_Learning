...
pipeline = Pipeline([('DT', DecisionTreeClassifier( min_samples_leaf=1,random_state=23))])
parameters = {
    'DT__criterion': ['gini','entropy'],
    'DT__max_depth': [4,5,6,7],
    'DT__max_features' :[15,18,20,25],
    'DT__min_samples_split' : [2,3,4],
    'DT__class_weight' :[{0:0.2, 1:0.8}, {0:0.3, 1:0.7}]
    }

#print(DecisionTreeClassifier().get_params().keys())
Grid_search = GridSearchCV(pipeline, parameters, scoring='precision',cv=6)
